...
def vk(method: str, params: dict) -> dict:
    ...
    try:
        ...
        data = r.json()
        if "error" in data:
            log.error(f"VK {method} {data['error']['error_code']}: {data['error']['error_msg']}")
        return data
...
def upload_photo_to_wall(group_id: int, filepath: str) -> str:
    ...
    try:
        ...
        with open(filepath, "rb") as f:
            res = requests.post(upload_url, files={"photo": f}, timeout=30).json()
...
def upload_avatar(group_id: int, filepath: str) -> bool:
    ...
    try:
        ...
        if "error" in r:
            return False
        with open(filepath, "rb") as f:
            res = requests.post(r["response"]["upload_url"], files={"photo": f}, timeout=30).json()
...
def publish_article(group_id: int, keyword: str, region: str,
    ...
    r = vk("articles.create", params)
    if "error" in r:
        log.warning(f"  articles.create ошибка: {r['error']} — публикуем как пост")
        return ""

    article_url = r["response"].get("view_url", "")
    log.info(f"  Статья создана: {article_url}")

    # Публикуем ссылку на статью на стене группы
    kw_tag  = keyword.replace(" ", "_")
    reg_tag = region.replace(" ", "_")
    post_text = (
        f"📚 {title}\n\n"
        f"Составили рейтинг лучших школ {region} для тех, кто ищет {keyword.lower()}.\n\n"
        f"Читать статью полностью 👇\n"
        f"{article_url}\n\n"
        f"#{kw_tag} #{reg_tag} #школа_{reg_tag} #образование_{reg_tag}"
    )
    vk("wall.post", {
        "owner_id":    f"-{group_id}",
        "message":     post_text,
...
def create_group(keyword: str, site: str, region: str) -> dict:
    ...
    # 7. URL — транслит ключа + регион
    screen_name = translit(keyword) + f"_{random.randint(10,99)}"
    r2 = vk("groups.editAddress", {"group_id": gid, "screen_name": screen_name})
    if "error" in r2:
